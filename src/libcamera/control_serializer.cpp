				 ...
				 ByteStreamBuffer &buffer)
{
	...
	for (const auto &ctrl : infoMap) {
		...
		entry.name_offset = values.offset();
...
ControlInfoMap ControlSerializer::deserialize<ControlInfoMap>(ByteStreamBuffer &buffer)
{
	...
	for (unsigned int i = 0; i < hdr->entries; ++i) {
		...
		if (entry->name_offset != values.offset()) {
			LOG(Serializer, Error)
				<< "Bad data, name offset mismatch (entry "
				<< i << ")";
			return {};
		}

		const auto *nameData = entry->name_len
            ? values.read<const uint8_t>(entry->name_len) : nullptr;
		if (entry->name_len && !nameData) {
