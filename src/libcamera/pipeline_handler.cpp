					...
					const Orientation &orientation)
{
	...
	std::stringstream o;
	o << orientation;
	configurationDict.scalar("orientation", o.str());
	...
	for (const auto &stream : streams) {
		...
		yamlStream.scalar("pixelformat", streamConfig.pixelFormat.toString());
		yamlStream.scalar("size", streamConfig.size.toString());
		yamlStream.scalar("stride", std::to_string(streamConfig.stride));
		yamlStream.scalar("frameSize", std::to_string(streamConfig.frameSize));
		yamlStream.scalar("bufferCount", std::to_string(streamConfig.bufferCount));

		if (streamConfig.colorSpace)
			yamlStream.scalar("colorSpace",
...
void PipelineHandler::dumpRequest(Request *request, DumpMode mode)
{
	...
	for (const auto &pair : controls) {
		...
		yamlCtrls.scalar(ctrlId->name(), pair.second.toString());
	}
}
