...
void RkISP1CameraData::paramsComputed(unsigned int frame, unsigned int bufferId,
				     ...
				     const ControlList &metadata)
{
	...
	/*
	 * bufferId can be 0, if ipa->processStats() was called
...
void PipelineHandlerRkISP1::stopDevice(Camera *camera)
{
	...
	/*
	 * There can still be requests that are either waiting for metadata or
	 * that contain buffers which were not yet queued at all.
	 */
...
void PipelineHandlerRkISP1::tryCompleteRequests()
{
	...
	while (!queuedRequests_.empty()) {
		...
		LOG(RkISP1Schedule, Debug) << "Complete request " << info.request->sequence()
					   << " frame " << info.sequence;
...
void PipelineHandlerRkISP1::imageBufferReady(FrameBuffer *buffer)
{
	...
	/* In the non-dewarper case, there must be a valid request */
	ASSERT(reqInfo);
	...
	int droppedFrames = imageSyncHelper_.receivedFrame(reqInfo->sequence, sequence);
	if (droppedFrames != 0)
		LOG(RkISP1Schedule, Warning)
	...
	auto &frameInfo = sensorFrameInfos_[sequence];
	if (frameInfo.metadataProcessed) {
		LOG(RkISP1Schedule, Debug)
			<< "Apply stored metadata " << sequence;
		request->_d()->metadata().merge(frameInfo.metadata);
	}

	if (frameInfo.statsLost) {
		LOG(RkISP1Schedule, Warning) << "Send empty stats to fill metadata for "
					     << sequence;
	...
	if (metadata.status != FrameMetadata::FrameCancelled) {
		...
		/* No need to block waiting for metadata on a cancelled frame. */
		frameInfo.metadataProcessed = true;
	}
...
void PipelineHandlerRkISP1::statBufferReady(FrameBuffer *buffer)
{
	...
	if (buffer->metadata().status == FrameMetadata::FrameCancelled) {
		/*
		 * Frame cancelled only happens after the ipa was stopped, so
		 * there must be no more buffers in postprocessing stage
		 */
		ASSERT(statQueue_->empty(BufferQueue::Postprocessing));
		return;
	}
	...
	while (nextStatsToProcess_ < sequence)
		sensorFrameInfos_[nextStatsToProcess_++].statsLost = true;
