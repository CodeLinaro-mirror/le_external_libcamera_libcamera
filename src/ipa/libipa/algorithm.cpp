 ...
 *
 * Care shall be taken to ensure that all values in \a frameContext that get
 * accessed from within process() are initialized. Examples for this are
 * exposure and gain. These should be initialized even if they
 * get updated again in prepare().
 * There are two reasons for this. In RAW mode prepare() is not called at all.
 * The other case is when a resynchronization happens. In that case it is not
 * guaranteed that prepare() is called for every frame but process() should
 * still be able to update the active state.
 */
