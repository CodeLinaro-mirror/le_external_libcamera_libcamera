		  ...
		  DebayerParams *params)
{
	...
	if (!frameContext.lsc.enabled) {
		if (lastAppliedCt_ != 0) {
			...
			LOG(IPASoftIspLsc, Debug)
				<< "LSC disabled (version " << params->lscLutVersion << ")";
		}
		return;
	}
	...
	LOG(IPASoftIspLsc, Debug)
		<< "LSC updated to version " << params->lscLutVersion
		<< " and temperature " << ct << " (was " << lastAppliedCt_ << ")";
