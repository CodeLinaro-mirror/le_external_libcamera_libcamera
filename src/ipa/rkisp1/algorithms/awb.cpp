...
{
public:
	RkISP1AwbStats(const IPAFrameContext &frameContext,
		       const rkisp1_stat_buffer *stats)
	{
		calculateRgbMeans(frameContext, stats);
...
private:
	void calculateRgbMeans(const IPAFrameContext &frameContext,
			       const rkisp1_stat_buffer *stats);
	bool rgbMode_ = false;
};
				       ...
				       const rkisp1_stat_buffer *stats)
{
	...
	if (rgbMode_) {
		...
		rgbMeans_ = yuv2rgbMatrix * (yuvMeans - yuv2rgbOffset);
	 ...
	 */
	rgbMeans_ = frameContext.ccm.ccm.inverse() * rgbMeans_;
	 ...
	 */
	rgbMeans_ /= frameContext.awb.gains.max(0.01);
}
...
{
	RkISP1AwbStats awbStats(frameContext, stats);
