 ...
 * regulation so that the number of nearly saturated pixels go below a given
 * threshold (controllable via WdrMaxBrightPixels, default is 2%) specified in
 * the tuning file is reached.
 *
...
void WideDynamicRange::applyHistogramEqualization(double strength)
{
	 ...
	 * impression is produced by the other algorithms. The target is that
