...
static double interpolateQuadatric(ipa::Pwl::PointF const &a, ipa::Pwl::PointF const &b,
				   ipa::Pwl::PointF const &c)
{
	/*
...
double Awb::coarseSearch(ipa::Pwl const &prior)
{
	...
	while (true) {
		...
		LOG(RPiAwb, Debug)
			...
			<< finalLogLikelihood;
		points_.push_back(ipa::Pwl::PointF({t, finalLogLikelihood}));
...
void Awb::fineSearch(double &t, double &r, double &b, ipa::Pwl const &prior)
{
	...
	ipa::Pwl::PointF transverse({bDiff, -rDiff});
	...
	for (int i = -nsteps; i <= nsteps; i++) {
		...
		ipa::Pwl::PointF points[maxNumDeltas];
		int bestPoint = 0;
		/* Take some measurements transversely *off* the CT curve. */
		for (int j = 0; j < numDeltas; j++) {
			points[j][0] = -config_.transverseNeg +
				       (transverseRange * j) / (numDeltas - 1);
			ipa::Pwl::PointF rbTest = ipa::Pwl::PointF({rCurve, bCurve}) +
