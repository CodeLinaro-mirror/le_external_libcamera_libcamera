...
BlackLevel::BlackLevel()
	: blackLevelHistIdx_(255), blackLevelSet_(false)
{
}
 ...
 */
unsigned int BlackLevel::get()
{
	return blackLevelSet_ ? blackLevelHistIdx_ * (256 / SwIspStats::kYHistogramSize) : 0;
}
...
void BlackLevel::update(SwIspStats::histogram &yHistogram)
{
	...
	for (unsigned int i = 0, seen = 0;
	     i < blackLevelHistIdx_ && i < SwIspStats::kYHistogramSize;
	     i++) {
		seen += yHistogram[i];
		if (seen >= pixelThreshold) {
			blackLevelHistIdx_ = i;
			blackLevelSet_ = true;
			LOG(IPASoftBL, Debug)
				<< "Auto-set black level: "
				<< blackLevelHistIdx_ << "/" << SwIspStats::kYHistogramSize
