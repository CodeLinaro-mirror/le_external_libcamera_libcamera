...
#
# The sweep sets V4L2 analogue_gain *codes* via v4l2-ctl (needed to measure a
# helper that does not exist yet). After a helper exists, emit a cam --script
# YAML to replay the same points as linear AnalogueGain (see --emit-cam-script
# and utils/measure-analogue-gain-cam.yaml).
#
# The fitter only targets AnalogueGainLinear { m0=0, c0=k, m1=-1, c1=k }
# i.e. G = k / (k - code). Other linear or exponential families are out of scope.
#
# Example:
...
#   ./utils/measure-analogue-gain.py --emit-cam-script /tmp/gain-measure/cam.yaml \
#       --gains 0,200,400,800 --model-k 1024 --exposure-us 10000
...
def emit_cam_script(
    ...
    """Write a cam --script YAML that steps linear AnalogueGain.

    Use this *after* a CameraSensorHelper exists, to verify libcamera applies
    the same G = k/(k-code) mapping. Bring-up of a new helper still needs
    v4l2-ctl codes (AnalogueGain is not available until the helper loads).
    """
    lines = [
        "# SPDX-License-Identifier: CC0-1.0",
        "#",
        "# Generated by utils/measure-analogue-gain.py --emit-cam-script",
        f"# AnalogueGain = {model_k}/({model_k}-code)  "
        "(AnalogueGainLinear { m0=0, c0=k, m1=-1, c1=k })",
        "#",
        "#   cam --camera 1 --stream role=raw,width=W,height=H \\",
        f"#       --script {path} --capture=N --file=out-#.bin",
        "#",
        "frames:",
    ]
    frame = 0
    for code in gains:
        if code >= model_k:
            continue
        gain = predicted_linear(code, model_k)
        lines.append(f"  - {frame}:")
        lines.append(f"      AnalogueGain: {gain:.6f}")
        if exposure_us is not None:
            lines.append(f"      ExposureTime: {exposure_us}")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        metavar="FILE",
        help=(
            "write a cam --script YAML that steps AnalogueGain as "
            "G=k/(k-code) and exit (no capture)"
        ),
    )
    parser.add_argument(
        "--exposure-us",
        type=int,
        default=None,
        help="optional ExposureTime (microseconds) for --emit-cam-script",
    )
    args = parser.parse_args()

    if args.emit_cam_script:
        emit_cam_script(
            args.emit_cam_script,
            parse_gains(args.gains),
            args.model_k,
            args.exposure_us,
        )
        print(f"Wrote {args.emit_cam_script}", flush=True)
        return 0
    ...
    script_path = out / "cam-script.yaml"
    emit_cam_script(script_path, gains, args.model_k, args.exposure_us)
    print(
